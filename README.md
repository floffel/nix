...
| `nixidm` | `/mnt/pve/nas/shared/secrets/mail` | `var/lib/secrets/mail` | `rw` | Provisions mail LDAP API token + nginx ldap.conf (sole writer) |
...
| `nixnginx` | `/mnt/pve/nas/shared/secrets/mail` | `var/lib/secrets/mail` | `ro` | Reads nginx ldap.conf generated by nixidm |
...
├── dovecot/            # rendered by nixmail's mail-ldap-config service
│   └── ldap-password.txt
└── postfix/            # rendered by nixmail's mail-ldap-config service
    ├── ldap-recipients.cf
    ├── ldap-aliases.cf
    ├── ldap-senders.cf
    ├── ldap-catchalls.cf
...
`nixidm` is the sole writer of `ldap-token` and `nginx-ldap.conf`.  `nixmail`
reads `ldap-token` in a `oneshot` service (`mail-ldap-config`) that runs before
Dovecot/Postfix and renders the consumer config files.  `nixnginx` includes
`nginx-ldap.conf` directly.  All three containers mount the mail secrets
directory: `nixidm` (rw), `nixmail` (rw), `nixnginx` (ro).
