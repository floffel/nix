...
*   `/var/lib/secrets/mail/nginx-ldap.conf`: The LDAP server configuration block, including the bindDN and API token password. This file is **auto-generated** by nixidm's `kanidm-mail-token` service on every Kanidm start and shared via the read-only mail secrets mount. No manual setup is required. See the root README's "Shared mail LDAP token" section for details.
