...

{
  ...
  networking.extraHosts = ''
    127.0.0.1 ldap
    127.0.0.1 minnecker.com mail.minnecker.com backendmail.minnecker.com
  ...
  # The testScript driver connects to 127.0.0.1 inside the VM for SMTP/
  # IMAP/DNS probes. Ensure a python interpreter exists on every node.
  ...
  systemd.services.test-secrets = {
    ...
    script = ''
      ...
      # DKIM private key for rspamd (dovecot/postfix milter target reads this
      # path; without it rspamd-adjacent signing fails and postfix milter
      # errors can delay delivery in tests).
      mkdir -p /var/lib/secrets/mail/dkim
      openssl genrsa -out /var/lib/secrets/mail/dkim/minnecker.com.private 1024 2>/dev/null
      chmod 640 /var/lib/secrets/mail/dkim/minnecker.com.private

      # parsedmarc IMAP/SMTP password (placeholder — parsedmarc is not
      # asserted on in the VM tests; the rspamd controller password is
      # generated by the mail-rspamd-password oneshot instead).
      mkdir -p /var/lib/secrets/mail/parsedmarc
      printf 'vm-placeholder' > /var/lib/secrets/mail/parsedmarc/imap-password
      chmod 600 /var/lib/secrets/mail/parsedmarc/imap-password
