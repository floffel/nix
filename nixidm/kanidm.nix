  ...
  services.kanidm = {
    ...
    provision = {
      ...
      systems.oauth2 = {
        ...
        # Vaultwarden SSO. Confidential client (SSO_CLIENT_ID +
        # SSO_CLIENT_SECRET) with PKCE additionally enabled on the consumer
        # side (SSO_PKCE=true). The redirect URI is auto-generated by
        # Vaultwarden from its DOMAIN setting as
        #   {DOMAIN}/identity/connect/oidc-signin
        # (see src/config.rs:generate_sso_callback_path). Vaultwarden has no
        # OIDC-driven admin mapping — admin is via ADMIN_TOKEN only — so no
        # *_admins group or claimMap is declared here, matching forgejo/matrix.
        vaultwarden = {
          displayName = "Vaultwarden (Password Manager)";
          originUrl = "https://vault.minnecker.com/identity/connect/oidc-signin";
