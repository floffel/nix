# NixOS Migration Configuration for Nginx and Roundcube (`nginxng`)
...
*   [`configuration.nix`](file:///root/nix/nginxng/configuration.nix): The main NixOS entry point, defining hostname and network configuration.
*   [`nginx.nix`](file:///root/nix/nginxng/nginx.nix): The service definition file containing the custom Nginx build (with NJS, Brotli, and LDAP), virtual hosts, upstreams, mail proxy block, and the Roundcube webmail configuration.
*   [`auth.js`](file:///root/nix/nginxng/auth.js): The Nginx JavaScript (njs) authentication script copied from `/etc/nginx/njs/auth.js`.
...
mp0: /tank/secrets/nginxng,mp=/var/lib/secrets/nginx,ro=1
